...
def negative_assert_symbol(first_name):
    ...
    user_response = sender_stand_request.post_new_user(response)
...
def negative_assert_no_firstname(first_name):
    #user_body = get_user_body(first_name)
    response = sender_stand_request.post_new_user(first_name)

    assert response.status_code == 400
    assert response.json()["code"] == 400
    assert response.json()["message"] == "No se han aprobado todos los parámetros requeridos"
    #No se enviaron todos los parámetros requeridos
...
#    positive_assert("Aasdfghjklzxcvb")
